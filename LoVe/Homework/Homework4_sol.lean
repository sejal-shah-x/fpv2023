...
lemma dotAndSwap (P Q R : Prop) (hpq : P ∧ Q ∧ R) : Q ∧ P := 
  And.intro ((hpq.right).left) (hpq.left)
...
def emp : SimpleQueue α := []

def enq (q : SimpleQueue α) (x : α) : SimpleQueue α :=
  q.append [x]

def deq (q : SimpleQueue α) : Option (α × SimpleQueue α) :=
  match q with
  | [] => none
  | h :: tq => some (h, tq)


def tst : SimpleQueue Nat := SimpleQueue.emp

#eval tst
#eval tst.enq 1
#eval ((tst.enq 1).enq 2).deq
#eval tst.deq
...
theorem emp_equiv {α : Type} :
  ...
  by
    rw [Equiv]
    rw [SimpleQueue.emp]
    rw [EfficientQueue.emp]
    have sq : SimpleQueue α :=
      SimpleQueue.emp
    have eq : EfficientQueue α :=
      EfficientQueue.emp
    have fst : List α := eq.fst
    have lst : List α := eq.snd
    have sqlst : List α := sq
    have eqlst : List α := 
      fst ++ lst.reverse
    simp
    done
...
theorem enq_equiv {α : Type} :
  ...
  by
    ...
    simp
    rw [SimpleQueue.enq]
    rw [EfficientQueue.enq]
    simp
    rw [←(List.append_assoc (eq.fst) (eq.snd.reverse) [x])]
    rw [←hleft]
    done
...
theorem deq_equiv {α : Type} :
  ∀ (sq : SimpleQueue α) (eq : EfficientQueue α),
  Equiv sq eq →
  (match sq.deq, eq.deq with
  | none, none => True
  | some (x, xs), some (y, ys) => (x = y) ∧ (Equiv xs ys)
  | _, _ => False
  ) := sorry
...
| HList.nil, hlb => hlb
| HList.cons x xs, hlb => HList.cons x (HList.append xs hlb)

#check HList.append H[14, true, "abc"] H[16, "false", false]
...
def columnwiseType : List Type → List Type
  | [] => []
  | x :: xs => (List x) :: (columnwiseType xs)

#check (columnwiseType [String, String, ℕ])
#check columnwiseType [ℕ, Bool]
...
def listHlistToHlistList :
  ...
  | [], lofrows => HList.nil
  | _ :: _, lofrows =>  
      HList.cons (lofrows.map HList.hd)
      (listHlistToHlistList (lofrows.map HList.tl))

def tst2 := [H["Providence", "RI", 2912], H["Pawtucket", "RI", 2860], H["Boston", "MA", 2110]]
#check listHlistToHlistList tst2
#eval listHlistToHlistList tst2
...
-- given a table with 2 columns: List<α>, List<β>
-- The length of List<α> (column 1) must equal the number of rows 
...
theorem length_hllh_false : ¬ length_hllh := 
  by
    rw [length_hllh]
    rw [Not]
    let ex : HList (columnwiseType [ℕ, Empty]) :=
      H[[10], []]
    intro lh
    have st := lh ℕ Empty ex
    have lenres :=
      calc
        List.length (hlistListToListHlist ex) = List.length (HList.hd ex) :=
          by apply st
        _ = 1 :=
          by rfl
    conv at st => {
    ...
    have elem : HList [ℕ, Empty] := pickFromSingletonList lenres
    have e2 : Empty := HList.hd (HList.tl elem)
    apply Empty.elim e2
    done
...
The function would produce a list, of type HList αs, with only those 'rows' that 
have an entry for every column.

i.e           List.length output = minimum (input.map List.length)

To elaborate, in a case where the lengths of the column lists don't match
...
An implementation would have one of two choices:
1. Only produce a List of every valid row 
      
      In this case that would produce
            ------------------------------
            | "Providence" | "RI" | 2912 |
            ------------------------------

2. Place filler values in the 'holes' of the table
            ----------------------------------
            | "Providence" | "RI" | 2912     |
            |  FILLER      | "RI" | 2860     |
            |  FILLER      | "MA" | FILLER   |
...
Case 1 is the only valid solution.

This is because of the instance where one of the columnwise types is Empty.

Case 2 would fail because there would be no filler value possible that would
still match the type HList[..., Empty, ...].

Case 1 would solve this vacuously by returning an empty list of type HList αs.

II.
We would need to assume/enforce that the length of every column list is the same.
i.e. In an HList where every entry is a List, each list representing a column, 
