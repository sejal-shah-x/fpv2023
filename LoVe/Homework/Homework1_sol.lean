...
def singletons {α : Type} : List α → List (List α)
  | [] => []
  | x :: xs => [x] :: singletons xs

-- #eval singletons [1, 2, 3, 4] -- expected: [[1], [2], [3], [4]]
-- #eval @singletons ℕ [] -- expected: []
...
theorem flatten_singletons {α : Type} (l : List α) : 
  flatten (singletons l) = l := 
  sorry
...
#eval singletons (flatten [[1], [], []]) -- result: [[1]] != orignal: [[1], [], []]
